## Привет！ 👋

QA Engineer / Manual / WEB / API / IOS / Android

📍 Новосибирск | 🏠 Готов к переезду(всё зависит от офера) | 💻 Удалёнка

📞 +7 (913) 481-35-55 (предпочтительно)

📧 chelovek.zakon@inbox.ru

💬 Telegram: @DeviIS1990

🐙 GitHub: github.com/DevilS1990

🌐 Сайт-визитка: https://devils1990.github.io


 СПЕЦИАЛИЗАЦИЯ: Интеграционное, микросервисное и API тестирование

Работал в кросс-функциональной команде из 6–8 разработчиков, 2 QA, аналитика и PM. Участвовал в полном цикле разработки: от анализа требований до релиза и мониторинга в продакшене

Стек: PostgreSQL, MongoDB, Kafka, REST/SOAP, Postman, WireMock, GitLab CI/CD, Test IT, JavaScript (автотесты)

---

КЛЮЧЕВЫЕ КОМПЕТЕНЦИИ:

· Типы тестирования: Интеграционное, E2E, Регрессионное, Исследовательское, Smoke, Системное.

· Области: Микросервисная архитектура (сервисы оплаты, авторизации, ачивок, PvP), Монолит, Клиент-сервер, Базы данных.

· Инструменты: Devtools, Postman (коллекции, окружения), Swagger, SoapUI, WireMock, Kafka, Jaeger, Kibana, Sentry, Grafana, Git, GitLab CI/CD, Charles Proxy, BrowserStack.

---

ПРОФЕССИОНАЛЬНЫЙ ОПЫТ:

Участие в разработке высоконагруженной микросервисной платформы с доменами: оплата, пользователи (авторизация), геймификация (ачивки, аватары) и PvP-взаимодействие.

ДОСТИЖЕНИЯ:

1. Оптимизация регрессионного тестирования: Разработал и внедрил систему коллекций в Postman с динамическими переменными окружений и набором скриптов во вкладке Tests, что сократило время прогона регрессионных сценариев API на 30%.

2. Ускорение подготовки тестовых данных: Автоматизировал создание сложных тестовых сценариев через Pre-request Scripts в Postman. Например, реализовал массовое создание юнитов (1 вместо 6 запросов) и их батч-загрузку в add-pokeboll, что сэкономило до 40% времени на подготовку данных для тестирования PvP-битв.

3. Ускорение дебаггинга: Внедрил практику анализа логов (Kibana) и трассировки (Jaeger) на этапе воспроизведения дефектов, что сократило среднее время расследования инцидентов на 20–30%.

4. Обеспечение параллельной разработки: Настроил заглушки (WireMock) для имитации поведения сервисов оплаты и ачивок, что позволило команде фронтенда начать тестирование на 2 недели раньше готовности бэкенда.

ОБЯЗАННОСТИ:

API И ИНТЕГРАЦИОННОЕ ТЕСТИРОВАНИЕ МИКРОСЕРВИСОВ:

· Проводил ручное и интеграционное тестирование распределенной системы (REST/SOAP API), проверяя корректность взаимодействия между сервисами оплаты, авторизации и ачивок.

· Проектировал и поддерживал коллекции в Postman, используя переменные окружений и скрипты для автоматизации проверок статус-кодов и схем ответов (JSON Schema).

· Тестировал асинхронное взаимодействие через Kafka: проверял доставку событий (например, после успешной оплаты отправляется событие в сервис ачивок) и обрабатывал сценарии с задержками.

· Использовал WireMock для создания заглушек зависимых сервисов, что позволяло тестировать пограничные состояния (таймауты, ошибки 5xx) и изолировать тестируемый компонент.

· Применял Jaeger для трассировки запросов при поиске проблем в микросервисной цепочке (например, при сбоях в PvP-битвах).

РАБОТА С БД И ЛОГАМИ:

· Выполнял прямую валидацию данных в PostgreSQL (использование JOIN, GROUP BY, агрегатных функций) и MongoDB для проверки консистентности данных после сложных транзакций (покупка аватаров, завершение битв).

· Интегрировал анализ систем логов в ежедневную практику: работа с Kibana для поиска ошибок по трейс-идам, Sentry для отслеживания эксепшенов на клиенте, Grafana для мониторинга метрик сервисов.

ФУНКЦИОНАЛЬНОЕ И КЛИЕНТ-СЕРВЕРНОЕ ТЕСТИРОВАНИЕ:

· Проводил сквозное (E2E) тестирование пользовательских путей: от UI (фронтенд) до проверки записи в БД.

· Использовал Chrome DevTools (Network, Application, Cookie, Local Storage) и Charles Proxy (подмена ответов, тестирование офлайн-режимов) для проверки обработки ошибок на клиенте и безопасности хранения данных.

· Участвовал в регрессионном тестировании фронтенда, проверяя вёрстку по макетам Figma и кросс-браузерную совместимость (BrowserStack).

ДОКУМЕНТИРОВАНИЕ И УЧАСТИЕ В ПРОЦЕССАХ:

· Разрабатывал и актуализировал тест-кейсы и тест-сьюты в Test IT для обеспечения полного регрессионного покрытия ключевых модулей.

· Составлял декомпозицию функционала в виде MindMap (Miro) для визуализации зон покрытия тестами.

· Оформлял баг-репорты в Яндекс.Трекер с высоким уровнем детализации (шаги, фактические и ожидаемые результаты, приложенные логи и скриншоты).

· Участвовал в ревью требований (аналитика), выявляя противоречия в логике на ранних этапах.

· Работал с Git (ветвление, разрешение конфликтов), участвовал в настройке CI/CD пайплайнов в GitLab (сборка стендов, прогон автотестов).

---

ТЕХНИЧЕСКИЕ НАВЫКИ:

· API: REST, SOAP

· Инструменты API: Postman (написание скриптов в Tests и Pre-request Scripts, коллекции, окружения, Newman), Swagger, SoapUI

· Базы данных: PostgreSQL (SQL запросы средней сложности: JOIN, GROUP BY), MongoDB

· Брокеры сообщений: Kafka (базовое понимание, проверка очередей событий)

· Мониторинг и Логи: Kibana, Sentry, Grafana, Jaeger

· Виртуализация/Заглушки: WireMock

· Инструменты разработчика: Chrome DevTools (Network, Application, Cookie, Local Storage), Charles Proxy (подмена ответов, тестирование таймаутов)

· CI/CD и VCS: Git, GitLab CI/CD (базовая настройка пайплайнов), Linux (команды для просмотра логов)

· Тестовая документация: Test IT, Яндекс.Трекер, Miro, Figma

· Скриптинг: JavaScript (написание скриптов в Postman, базовые автотесты для критических сценариев)

## Тестирование API и интеграций
<div>
<img src="https://user-images.githubusercontent.com/25181517/192109061-e138ca71-337c-4019-8d42-4792fdaa7128.png" title="Postman" alt="Postman" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/192107860-9a9f0894-0e34-4ab3-964d-6297ee4c00e9.png" title="SOAP" alt="SOAP" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/192107004-2d2fff80-d207-4916-8a3e-130fee5ee495.png" title="Kafka" alt="Kafka" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/192107858-fe19f043-c502-4009-8c47-476fc89718ad.png" title="REST" alt="REST" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/186711335-a3729606-5a78-4496-9a36-06efcc74f800.png" title="Swagger" alt="Swagger" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/117207330-263ba280-adf4-11eb-9b97-0ac5b40bc3be.png" title="Docker" alt="Docker" width="40" height="40"/>&nbsp
</div>

## Тестирование Web и Мобильных приложений
<div>
<img src="https://user-images.githubusercontent.com/25181517/189715289-df3ee512-6eca-463f-a0f4-c10d94a06b2f.png" title="Figma" alt="Figma" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/192107854-765620d7-f909-4953-a6da-36e1ef69eea6.png" title="HTTP" alt="HTTP" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/192158954-f88b5814-d510-4564-b285-dff7d6400dad.png" title="HTML" alt="HTML" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/183898674-75a4a1b1-f960-4ea9-abcb-637170a00a75.png" title="CSS" alt="CSS" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/189716855-2c69ca7a-5149-4647-936d-780610911353.png" title="Firebase" alt="Firebase" width="40" height="40"/>&nbsp
<img src="https://user-images.githubusercontent.com/25181517/192108895-20dc3343-43e3-4a54-a90e-13a4abbc57b9.png" title="Android Studio" alt="Android Studio" width="40" height="40"/>&nbsp
<img src="https://cdn.icon-icons.com/icons2/3053/PNG/512/charles_proxy_macos_bigsur_icon_190302.png" title="charles-proxy" alt="charles-proxy" width="40" height="40"/>&nbsp
</div>

## Логи и мониторинги
<div>
<img src="https://raw.githubusercontent.com/maliceio/kibana/master/docs/kibana-logo.png" title="Kibana" alt="Kibana" width="40" height="40"/>&nbsp
<img src="https://raw.githubusercontent.com/tandpfun/skill-icons/main/icons/Sentry.svg" title="Sentry" alt="Sentry" width="40" height="40"/>&nbsp
<img src="https://raw.githubusercontent.com/tandpfun/skill-icons/main/icons/Grafana-Dark.svg" title="Grafana" alt="Grafana" width="40" height="40"/>&nbsp
<img src="https://avatars.githubusercontent.com/u/28545596?s=200&v=4" title="Jaeger" alt="Jaeger" width="40" height="40"/>&nbsp
<img src="https://raw.githubusercontent.com/tandpfun/skill-icons/main/icons/Bash-Dark.svg" title="Bash" alt="Bash" width="40" height="40"/>&nbsp
</div>

## Тестовая документация 
<div>
<img src="https://is1-ssl.mzstatic.com/image/thumb/Purple116/v4/4b/7d/d4/4b7dd461-16e6-e245-af56-512fc8aa21e5/AppIcon-0-0-1x_U007emarketing-0-7-0-85-220.png/460x0w.webp" title="Yandex Tracker" alt="Yandex Tracker" width="40" height="40"/>&nbsp
<img src="https://logosandtypes.com/wp-content/uploads/2023/03/clickup.svg" title="ClickUp" alt="ClickUp" width="40" height="40"/>&nbsp
<img src="https://docs.testit.software/images/testit_logo_icon_blue.png" title="Test It" alt="Test It" width="40" height="40"/>&nbsp
<img src="https://raw.githubusercontent.com/tandpfun/skill-icons/main/icons/Notion-Dark.svg" title="Notion" alt="Notion" width="40" height="40"/>&nbsp
</div>

## Работа с базами данных
<div>
<img src="https://raw.githubusercontent.com/tandpfun/skill-icons/main/icons/PostgreSQL-Dark.svg" title="PostgreSQL" alt="PostgreSQL" width="40" height="40"/>&nbsp
<img src="https://raw.githubusercontent.com/tandpfun/skill-icons/main/icons/MongoDB.svg" title="Mongo DB" alt="Mongo DB" width="40" height="40"/>&nbsp
<img src="https://cdn.worldvectorlogo.com/logos/metabase.svg" title="Metabase" alt="Metabase" width="40" height="40"/>&nbsp
<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b5/DBeaver_logo.svg/512px-DBeaver_logo.svg.png" title="DBeaver" alt="DBeaver" width="40" height="40"/>&nbsp
</div>


## 🎒 Образование
<img width="400" height="1000" alt="08c1442714c" src="https://github.com/user-attachments/assets/5d1fa00b-c845-44cd-b87b-e728b3a142a1" />
<img width="400" height="1000" alt="мидл" src="https://github.com/user-attachments/assets/bfed1500-064a-464b-927b-118406d36e19" />
<img width="400" height="1000" alt="Сертификат_ИТ_компетенций" src="https://github.com/user-attachments/assets/d98a4363-807d-43b6-ae5b-edc0e78d7eb6" />
<img width="400" height="1000" alt="Функциональное_тестирование" src="https://github.com/user-attachments/assets/9ec03a15-f712-41bd-b79b-1b9688911e7e" />
<img width="400" height="1000" alt="Сертификат CSS" src="https://github.com/user-attachments/assets/4d175b35-f798-41f6-8cec-ded039dcad15" />
<img width="400" height="1000" alt="Сертификат ООП" src="https://github.com/user-attachments/assets/d7fc50aa-3843-45fd-a0e8-d8f2c8c52034" />
<img width="400" height="1000" alt="Сертификат SQL" src="https://github.com/user-attachments/assets/923439a6-186d-48d9-b0d8-f5d40050d784" />
<img width="400" height="1000" alt="Сертификат Docker" src="https://github.com/user-attachments/assets/25e8d01b-a55c-44d3-be1b-a18a57092a01" />
<img width="400" height="1000" alt="Сертификат Git" src="https://github.com/user-attachments/assets/b456717f-bdf0-4474-a34a-a7a680ca391a" />
<img width="400" height="1000" alt="Сертификат Linux" src="https://github.com/user-attachments/assets/3fc97aee-dbbe-41fa-a0a6-add9926fdd9a" />
<img width="400" height="1000" alt="Сертификат JavaSkript" src="https://github.com/user-attachments/assets/08ed184d-3d1e-4a94-a446-338b39181ab4" />
<img width="400" height="1000" alt="Сертификат PostageSQL" src="https://github.com/user-attachments/assets/25559338-5673-49fa-b416-aeb0c9ee2113"/>
<img width="400" height="1000" alt="HTML" src="https://github.com/user-attachments/assets/58e28604-ce16-487e-b79b-b129a77369bb" />





<br>
<br>

## 🤝 Контакт
[![Email](https://img.shields.io/badge/Email-3b5998?style=flat-square&logo=Mail.Ru&logoColor=white)](mailto:devils1990@yandex.ru)
[![Telegram Badge](https://img.shields.io/badge/-Telegram-0088cc?style=flat-square&logo=Telegram&logoColor=white)](https://t.me/devils1990)

<!-- Документация по виджету контактов ↑↑ https://shields.io/badges --> 
